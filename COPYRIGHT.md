...
The maintainer-only renderer `scripts/render_12s_example.py` uses Pillow 11.3.0 and the platform binary supplied by imageio-ffmpeg 0.6.0: FFmpeg 7.1 on the approved macOS build host and FFmpeg 7.0.2-static on the Linux CI host. The exact local executable was built with `--enable-gpl --enable-libx264`, so that combined tool is governed by GPL version 2 or later; libx264's source is GPL version 2. imageio-ffmpeg's Python wrapper is BSD-2-Clause. The [FFmpeg legal page](https://ffmpeg.org/legal.html), [FFmpeg 7.1 source](https://ffmpeg.org/releases/ffmpeg-7.1.tar.xz), [FFmpeg 7.0.2 source](https://ffmpeg.org/releases/ffmpeg-7.0.2.tar.xz), [x264 source and COPYING file](https://code.videolan.org/videolan/x264/-/blob/master/COPYING), and the exact platform tool identities are recorded in [`references/copyright-and-provenance.md`](references/copyright-and-provenance.md). Those build tools, binaries, and the macOS system fonts used to flatten the text are not included and are not runtime dependencies of the website or Skill.
