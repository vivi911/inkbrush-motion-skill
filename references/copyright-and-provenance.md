...
| `assets/brush-poses-v4/pose-01.png` through `pose-09.png` | OpenAI ImageGen 3×3 action sheet, corrected on 2026-08-22 to remove rope-like extensions, then deterministically cropped, aligned, chroma-keyed, edge-finished, and given code-authored soft-hair pressure states by `scripts/prepare_nine_action_sprites.py`; exact source and published hashes in [`image-generation-record.md`](image-generation-record.md) | OpenAI assigns output rights to the user to the extent permitted by law; the selected sprites and code-authored reconstruction are published under MIT to the extent controlled and copyrightable, without claiming the depicted hand is a real contributor or identified person | Included |
